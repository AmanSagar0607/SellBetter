...
import { BadgeCheck, MoreVerticalIcon, ShoppingCart, Sparkles, Heart, Info, PlusCircleIcon } from 'lucide-react';
...
function ProductCardItem({ product, editable = false }) {
    ...
    const handleAddToCart = async (e) => {
        ...
        try {
            ...
            // Add to cart immediately for optimistic update
            const productWithId = { 
                ...product, 
                productId: product.id,
                cartId: Date.now() // Temporary ID for optimistic update
            };
            ...
            if (!response.data?.success) {
                ...
                setCart(prevCart => prevCart.filter(item => item.cartId !== productWithId.cartId));
                toast.error(response.data?.error || 'Failed to add to cart');
            } else {
                // Update with real cart ID
                setCart(prevCart => 
                    prevCart.map(item => 
                ...
                <div className="aspect-square rounded-lg overflow-hidden bg-gray-800">
                    <div className="relative w-full h-full">
                        ...
                        <Button
                            size="sm"
                            variant="outline"
                            className={`absolute top-3 right-3 p-2 h-8 w-8 rounded-full border-pink-500/20 transition-all backdrop-blur-sm ${isFavorite
                    ...
                    <div className="flex items-center justify-between gap-3">
                        ...
                        {!editable && (
                            <Button
                                ...
                                disabled={isLoading}
                                className="bg-pink-500/10 text-pink-400 hover:bg-pink-500/20 hover:text-pink-500 border border-pink-500/20 transition-all flex items-center justify-center gap-2 px- py-2 rounded-xl font-medium">

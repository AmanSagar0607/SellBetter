...
function CartList({ children }) {
    ...
    const groupedCart = useMemo(() => Object.values(groupCartItems), [groupCartItems]);
    ...
    const removeAllInstances = async (productId) => {
        ...
        try {
            const response = await axios.delete(`/api/cart?userId=${user.id}&productId=${productId}`);
    ...
    if (!isLoaded || isLoading) {
        return (
            <Sheet>
                ...
                <SheetContent className="bg-black border-white/10 w-full sm:w-[440px] flex flex-col h-full" side="right">
        ...
        <Sheet>
            ...
            <SheetContent className="bg-black border-white/10 w-full sm:w-[440px] flex flex-col h-full" side="right">
                <SheetHeader className="mb-6 relative flex-shrink-0">
                    ...
                    <SheetTitle className="text-white">Cart</SheetTitle>
                    <SheetDescription className="text-white/70">
                        Your cart has {cartCalculations.uniqueItemCount} {cartCalculations.uniqueItemCount === 1 ? 'item ' : 'items '}
                ...
                <div className="flex-1 min-h-0">
                    {cart.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full">
                            ...
                            <h3 className="text-white/70 text-lg font-medium">Your cart is empty</h3>
                ...
                {cart.length > 0 && (
                    <div className="border-t border-white/10 pt-4 mt-4 flex-shrink-0 bg-black">
